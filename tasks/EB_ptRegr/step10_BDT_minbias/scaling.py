...
pt = df["TkEle_pt"].values
hwQual = df["TkEle_hwQual"].values
gen = df["TkEle_Gen_pt"].values
ptCorr = df["TkEle_pt"].values * model.predict(df[features].values)
...
for idx,cut in enumerate(pt_cuts):
    ...
    fig.savefig(f"plots/turn_on_{cut:.2f}.pdf", bbox_inches="tight")
    fig.savefig(f"plots/turn_on_{cut:.2f}.png", bbox_inches="tight")
...
fig.savefig("plots/online_to_offline_scaling.pdf", bbox_inches="tight")
fig.savefig("plots/online_to_offline_scaling.png", bbox_inches="tight")
# %%
